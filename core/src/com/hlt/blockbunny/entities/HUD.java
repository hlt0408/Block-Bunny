...
public class HUD {
    ...
    public void render(SpriteBatch sb) {
        ...
        if ((bits & BIT_RED) != 0) { sb.draw(blocks[0], 40, 208); }
        if ((bits & BIT_GREEN) != 0) { sb.draw(blocks[1], 40, 208); }
        if ((bits & BIT_BLUE) != 0) { sb.draw(blocks[2], 40, 208); }

        // draw crystal
        sb.draw(crystal, 100, 208);

        // draw crystal amount
        drawString(sb, player.getNumCrystals() + " / " +
        player.getTotalCrystals(), 132, 211);
